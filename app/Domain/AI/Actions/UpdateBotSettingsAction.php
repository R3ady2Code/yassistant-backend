...
final class UpdateBotSettingsAction extends AbstractAction
{
    ...
    {
        return DB::transaction(function () use ($settings, $data): BotSettings {
            $fields = [];
            ...
            if ($data->operations !== null) {
                foreach ($data->operations as $operation => $isEnabled) {
                    BotSettingOperation::updateOrCreate(
                        [
                            'bot_setting_id' => $settings->id,
                            'operation' => $operation,
                        ],
            ...
            return $settings->load('operations')->refresh();
        });
    }
}
