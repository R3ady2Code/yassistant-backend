...
```
Telegram/WhatsApp → Backend (Laravel) → OpenAI/YClients
                         ↕
                React SPA (Frontend)
```

Backend — центральный сервис. Напрямую принимает Telegram webhook и отправляет сообщения через Telegram Bot API. Обрабатывает AI-логику, управляет записями через YClients, отдаёт данные во Frontend.
...
│   ├── Telegram/           # Telegram Bot API (webhook, отправка, парсинг)
...
│   ├── Analytics/
│   ├── Billing/
│   ├── Webhook/            # POST /api/webhook/telegram/{channel} (Telegram → Laravel)
│   └── Middleware/
├── Console/Commands/
└── Providers/
...
- (для Telegram) ngrok / Cloudflare Tunnel для публичного доступа к Laravel
...
#    OPENAI_API_KEY=sk-...
#    APP_URL=https://xxx.ngrok-free.app (публичный URL для Telegram webhook)
...
| `APP_URL` | Публичный URL приложения (для Telegram webhook) | `https://yourservice.com` |
...
Webhook:        POST /api/webhook/telegram/{channel}   (Telegram → Backend)
```
